...
abstract class CommonAsyncTask extends AsyncTask<Void, Void, Boolean> {
  ...
  final DriveSample activity;
  final Drive client;
  private final View progressBar;

  CommonAsyncTask(DriveSample activity) {
  ...
  protected final Boolean doInBackground(Void... ignored) {
    try {
      ...
      activity.startActivityForResult(
          userRecoverableException.getIntent(), DriveSample.REQUEST_AUTHORIZATION);
    } catch (IOException e) {
      Utils.logAndShow(activity, DriveSample.TAG, e);
    }
    return false;
  }
